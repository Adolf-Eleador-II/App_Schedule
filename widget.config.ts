...
import type { Widget, WithAndroidWidgetsParams } from 'react-native-android-widget';
import {
  AndroidConfig,
  withAndroidManifest,
  withDangerousMod,
...
export default function withAndroidWidgets(config: ExpoConfig, params: WithAndroidWidgetsParams) {
  let projectPaths: ProjectPaths = {
    platformProjectRoot: '',
    projectRoot: '',
  };

  config = withAndroidManifest(config, (androidManifestConfig: ExportedConfigWithProps<AndroidConfig.Manifest.AndroidManifest>) => {
      const mainApplication = getMainApplicationOrThrow(androidManifestConfig.modResults);
      ...
      params.widgets.forEach((widget) => { withWidgetReceiver(androidManifestConfig, mainApplication, widget) });
...
function withCollectionService(mainApplication: AndroidConfig.Manifest.ManifestApplication): void {
  mainApplication.service = mainApplication.service ?? [];

  const alreadyAdded = mainApplication.service.some(
    (service) => service.$['android:name'] === 'com.reactnativeandroidwidget.RNWidgetCollectionService'
);
...
function withConfigurableActivity(mainApplication: AndroidConfig.Manifest.ManifestApplication, params: WithAndroidWidgetsParams) {
  const hasConfigurableWidget = params.widgets.some((widget) => !!widget.widgetFeatures);

  if (hasConfigurableWidget) {
    mainApplication.activity = mainApplication.activity ?? [];
    const alreadyAdded = mainApplication.activity.some((activity) => activity.$['android:name'] === '.WidgetConfigurationActivity');
    if (alreadyAdded) return;
...
function withWidgetReceiver(config: ExpoConfig, mainApplication: AndroidConfig.Manifest.ManifestApplication, widget: Widget): void {
...
function withFonts(config: ExpoConfig, androidManifestConfig: ProjectPaths, fonts: ResourcePath[]) {
...
function withWidgets(config: ExpoConfig, projectPaths: ProjectPaths, widgets: Widget[]) {
...
function withConfigurableActivityClass(config: ExpoConfig, projectPaths: ProjectPaths, widgets: Widget[]) {
...
function withWidget(config: ExpoConfig, projectPaths: ProjectPaths, widget: Widget) {
  withDangerousMod(config, [
    'android',
    (dangerousConfig) => {
...
function withWidgetProviderClass(config: ExpoConfig, projectPaths: ProjectPaths, widget: Widget) {
...
function withWidgetProviderXml(config: ExpoConfig, projectPaths: ProjectPaths, widget: Widget) {
